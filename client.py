...
class CifarClient(fl.client.Client):
    ...
    def fit(self, ins: FitIns) -> FitRes:
        ...
        utils.train(self.model, self.data.trainloader, self.data.dataset_sizes, epochs=epochs, device=DEVICE)
        #utils.test(self.model, self.data.testloader, self.data.gallery_meta, self.data.query_meta, epochs=epochs, device=DEVICE)

        # Return the refined weights and the number of examples used for training
        weights_prime: Weights = get_weights(self.net)
        params_prime = fl.common.weights_to_parameters(weights_prime)
        num_examples_train = len(self.trainset)
        #metrics = {"duration": timeit.default_timer() - fit_begin}
        return FitRes(
            parameters=params_prime, num_examples=num_examples_train
        )
...
# arguments for testing federated model
    ...
    data = utils.Data(args.batch_size, args.erasing_p, args.color_jitter, args.train_all)
    data.preprocess()
