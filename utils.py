...
def extract_feature(model, dataloaders, ms):
    ...
    for data in dataloaders:
        img, label = data
        n, c, h, w = img.size()
        ff = torch.FloatTensor(n, 144).zero_().cuda()
...
def train(
    ...
    optimizer = get_optimizer(model, self.lr)
    ...
    batch_size=10
    train_criterion = NTD_Loss(144, 3, 1)
...
def test(
    ...
    print("Start Testing!")
    print("="*10)
    #print('We use the scale: %s'%multiple_scale)

    with torch.no_grad():
        gallery_feature = extract_feature(model, testloader['gallery'], multiple_scale)
        query_feature = extract_feature(model, testloader['query'], multiple_scale)
    ...
    for i in range(len(query_label)):
        ap_tmp, CMC_tmp = evaluate(query_feature[i], query_label[i], query_cam[i], gallery_feature, gallery_label[i], gallery_cam)
        if CMC_tmp[0]==-1:
            continue
        CMC = CMC + CMC_tmp
        ap += ap_tmp
